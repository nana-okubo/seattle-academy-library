...
public class AccountController {
    ...
    public String createAccount(Locale locale,
        ...
        // バリデーションチェック
        boolean isEmailVoid = email.matches("^([a-zA-Z0-9])+([a-zA-Z0-9\\._-])*@([a-zA-Z0-9_-])+([a-zA-Z0-9\\._-]+)+$");
        boolean isValidPW = password.matches("^[A-Za-z0-9]+$");
        boolean isVolidPWOForCheck = passwordForCheck.matches("^[A-Za-z0-9]+$");

        if (!isEmailVoid || !isValidPW || !isVolidPWOForCheck) {
            //  遷移して欲しい画面の名前
